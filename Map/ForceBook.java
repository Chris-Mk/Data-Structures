...
public class ForceBook {
    public static void main(String[] args) {
        ...
        String input;
        while (!"Lumpawaroo".equals(input = scan.nextLine())) {
            String[] data = Arrays
                    .stream(input.split("\\s+\\|\\s+|\\s+->\\s+"))
                    .toArray(String[]::new);
        ...
        forceBook.entrySet()
                ...
                .forEach(kvp -> {System.out.println(String.format("Side: %s, Members: %d",
                        kvp.getKey(),
                        kvp.getValue().size()));

                kvp.getValue().forEach(val -> {
